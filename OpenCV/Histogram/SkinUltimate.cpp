...
SkinUltimate::SkinUltimate(string posPath, string negPath)
{
	...
	for (b = 0; b < DIM; b++) {
		for (g = 0; g < DIM; g++) {
			for (r = 0; r < DIM; r++) {
				if (numberS[b][g][r] != 0) {
					skinMap[b][g][r] = (Ps*numberS[b][g][r]) / (Ps*numberS[b][g][r] + (1 - Ps)*numberN[b][g][r] * (sums / sumn));	//Ps_c = (Pc_s*Ps) / (Pc_s*Ps + Pc_ns(1-Ps));
	...
	for (b = 0; b < DIM; b++) {
		for (g = 0; g < DIM; g++) {
			for (r = 0; r < DIM; r++) {
				//Trees
				if ((r > 95) && (g > 40) && (b > 20) && (r > b) && (r > g + 15) && (r > min(b, g) + 15)) {
					map[TREE][b][g][r] = 255;
...
cv::Mat SkinUltimate::doIt(cv::Mat target, int type = ALL)
{
	if (type != ALL) {
...
{
	if (type < GRAY_TREE) {
		return map[type][b][g][r];
	}
	else {
		return r*(map[type - 4][b][g][r] / 255);
	}
	return 127;
}
